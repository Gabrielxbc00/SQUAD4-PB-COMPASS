...
movie_id = 335984
api_key = '09932c18c7a2cf9d89fc2fbe7c88d32b'
...
def lambda_handler(event, context):
    def obter_detalhes_filme(filme_id):
        url = f"{base_url}/{filme_id}"
        parametros_url = {'api_key': api_key}
        resposta = requests.get(url, params=parametros_url)
        return resposta.json()

    def obter_avaliacoes_filme(filme_id):
        url = f"{base_url}/{filme_id}/reviews"
        parametros_url = {'api_key': api_key}
        resposta = requests.get(url, params=parametros_url)
        return resposta.json()

    detalhes_filme = obter_detalhes_filme(movie_id)

    avaliacoes_filme = obter_avaliacoes_filme(movie_id)

    detalhes_filme['avaliacoes'] = avaliacoes_filme

    tempo_atual = datetime.datetime.now().strftime("%Y/%m/%d")
    chave_arquivo = f"raw/tmdb/json/{tempo_atual}/blade_runner_2049_assessments.json"
    s3_client.put_object(
        Bucket=BUCKET_NAME,
        Key=chave_arquivo,
        Body=json.dumps(detalhes_filme),
        ContentType='application/json'
    )

    return {
        'statusCode': 200,
        'body': json.dumps('Detalhes e avaliações do filme Blade Runner 2049 inseridos com sucesso no S3!')
    }
